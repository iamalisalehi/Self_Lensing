...
fdd=open("./files/Chain1.dat","r")
nns=sum(1 for line in fdd)
Chain= np.zeros((nns,NDIM+1))
Chain=np.loadtxt("./files/Chain1.dat")  
...
save2=open("./files/BestsNb.txt","w")
...
fig3.savefig("./Figs/BestModelnb.jpg", dpi=200)
...
'''
fig = corner.corner(samples1, show_titles=True,title_kwargs={"fontsize":22},labels=nam,label_kwargs={"fontsize":22},range=[(31.0, 45.0), (0.78, 0.85), (1.35, 1.6), (0.2, 0.9),(3.4,3.85)], ticks_kwargs={"fontsize":24},quantiles=[0.16,0.5,0.84],plot_datapoints=True)##, truths=b,truth_color="m")

#

fig.savefig("./Figs/trianglenb.jpg", dpi=200)
print(">>>>>>>>>>>>>>>> The triangle plot was made <<<<<<<<<<<<<<<<<<<<<<<<")
################################################################################
'''

test=open("./files/topsb.txt","w")
test.close()
nl=int(250)
...
for j in range(nns):
    i=j*120
    chi= float(Chain[i,NDIM]*-2.0)
    if(abs(chi)<4200.0 and k<nl):
        bt= Chain[i,:NDIM]
        chi2t,Fluxt[:,:,k]=orbit( Chain[i,:NDIM]  , tim, Flux, errf ,0)
        chi2m[k]=abs(Chain[i,NDIM]*2.0)##abs(chi2t)
        print("parameters:  ",    abs(Chain[i,NDIM]*2.0),  abs(chi2t) )
        print(Chain[i,:])
        print("*******************************************************************")  
        test=open("./files/topsb.txt","a+")
        sav=np.array([i*4,  bt[0],   bt[1],    bt[2],   bt[3],   bt[4],   abs(Chain[i,NDIM]*2.0)   ])
...
fig3=plt.gcf()
fig3.tight_layout(pad=0.2)
fig3.savefig("./Figs/top200nb.jpg", dpi=200)
